...
internal class WorkoutPresenter2 : WorkoutTimer2.TimerCallback {
    ...
    class WorkoutPresenterState(val workout: Workout2,
                                val exercisePos: Int,
                                val timeRemaining: Int,
                                val isTimerRunning: Boolean,
                                val stage: Stage) : Parcelable {
        constructor(parcel: Parcel) : this(
                parcel.readParcelable(Workout2::class.java.classLoader),
                parcel.readInt(),
                parcel.readInt(),
    ...
    private val workout: Workout2
    private val timer: WorkoutTimer2

    private var views = ArrayList<WeakReference<WorkoutContract2.View>>(3)
    ...
    constructor(workout: Workout2, vararg callbacks: WeakReference<WorkoutContract2.View>) {
        ...
        timer = WorkoutTimer2(this, prepLength)
    ...
    constructor(savedState: Parcelable, vararg callbacks: WeakReference<WorkoutContract2.View>) {
        ...
        timer = WorkoutTimer2(this, savedState.timeRemaining)
    ...
    private fun rerender(renderViews: Iterable<WeakReference<WorkoutContract2.View>>) {
    ...
    private fun renderStage(renderViews: Iterable<WeakReference<WorkoutContract2.View>>) {
        when (stage) {
    ...
    private fun renderPausePlay(renderViews: Iterable<WeakReference<WorkoutContract2.View>>) {
        if (timer.running) for (view in renderViews) view.get()?.setPlaying()
        else for (view in renderViews) view.get()?.setPaused()
    }

    private fun renderSeconds(renderViews: Iterable<WeakReference<WorkoutContract2.View>>) {
    ...
    fun renderToView(view: WorkoutContract2.View) {
        rerender(Collections.singleton(WeakReference(view)))
    }

    fun addViews(vararg callbacks: WeakReference<WorkoutContract2.View>) {
        views.addAll(callbacks)
        rerender(callbacks.asIterable())
    }

    fun removeView(view: WorkoutContract2.View) {
