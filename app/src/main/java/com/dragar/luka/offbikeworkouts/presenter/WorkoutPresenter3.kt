...
internal class WorkoutPresenter3 : WorkoutTimer3.TimerCallback {
    ...
    class WorkoutPresenterState(val workout: Workout3,
                                val exercisePos: Int,
                                val timeRemaining: Int,
                                val isTimerRunning: Boolean,
                                val stage: Stage) : Parcelable {
        constructor(parcel: Parcel) : this(
                parcel.readParcelable(Workout3::class.java.classLoader),
                parcel.readInt(),
                parcel.readInt(),
    ...
    private val workout: Workout3
    private val timer: WorkoutTimer3

    private var views = ArrayList<WeakReference<WorkoutContract3.View>>(3)
    ...
    constructor(workout: Workout3, vararg callbacks: WeakReference<WorkoutContract3.View>) {
        ...
        timer = WorkoutTimer3(this, prepLength)
    ...
    constructor(savedState: Parcelable, vararg callbacks: WeakReference<WorkoutContract3.View>) {
        ...
        timer = WorkoutTimer3(this, savedState.timeRemaining)
    ...
    private fun rerender(renderViews: Iterable<WeakReference<WorkoutContract3.View>>) {
    ...
    private fun renderStage(renderViews: Iterable<WeakReference<WorkoutContract3.View>>) {
        when (stage) {
    ...
    private fun renderPausePlay(renderViews: Iterable<WeakReference<WorkoutContract3.View>>) {
        if (timer.running) for (view in renderViews) view.get()?.setPlaying()
        else for (view in renderViews) view.get()?.setPaused()
    }

    private fun renderSeconds(renderViews: Iterable<WeakReference<WorkoutContract3.View>>) {
    ...
    fun renderToView(view: WorkoutContract3.View) {
        rerender(Collections.singleton(WeakReference(view)))
    }

    fun addViews(vararg callbacks: WeakReference<WorkoutContract3.View>) {
        views.addAll(callbacks)
        rerender(callbacks.asIterable())
    }

    fun removeView(view: WorkoutContract3.View) {
