...
import com.dragar.luka.offbikeworkouts.presenter.WorkoutService2
import com.dragar.luka.offbikeworkouts.view.WorkoutPagerAdapter2.Companion.SUCCESS_INDEX_FROM_END
import com.dragar.luka.offbikeworkouts.view.WorkoutPagerAdapter2.Companion.WORKOUT_INDEX_FIRST
import kotlinx.android.synthetic.main.activity_workout2.*
...
class WorkoutActivity2 : AppCompatActivity(), ServiceConnection, WorkoutContract2.View {
    companion object {
    ...
    private inner class PausePlayListener : View.OnClickListener {
        override fun onClick(p0: View?) {
            this@WorkoutActivity2.presenterService?.get()?.togglePlayPause()
        }
    }
    ...
    private var presenterService: WeakReference<WorkoutService2>? = null
    private var restoredState: Parcelable? = null
    private var ttsEnabled = false
    private var workout: Workout2? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        workout = intent.getParcelableExtra(WORKOUT_KEY2)
//        workout?.customColor?.toInt()?.let { headerBox.setBackgroundColor(it) } //todo make sure color is applied on restore as well
    ...
    override fun onStart() {
        ...
        bindService(
                Intent(this, WorkoutService2::class.java),
    ...
    override fun onStop() {
        super.onStop()
        startService(Intent(this, WorkoutService2::class.java)) //todo perhaps not at all necessary
    ...
    override fun onServiceConnected(p0: ComponentName?, binder: IBinder) {
        presenterService = (binder as WorkoutService2.WorkoutBinder).service
        presenterService?.let {
            workoutPagerOSVP.adapter = WorkoutPagerAdapter2(supportFragmentManager, workout!!) //todo get workout from binder, not from activity, maybe (though I use the workout here to start the service, so maybe not? but the service may have a different workout)
    ...
    private fun setOnClickListeners(service: WorkoutService2?) {
        runOnUiThread {
    ...
    private val curWorkoutFragment: WorkoutFragment2?
        get() { //todo too inefficient
            val curFragment = (workoutPagerOSVP.adapter as WorkoutPagerAdapter2)
                    .getFragment(workoutPagerOSVP.currentItem) as WorkoutFragment2?
            if (curFragment == null) {
                needsRerender = true
            }
    ...
    override fun setExercise(workoutPos: Int, exerciseMeta: ExerciseMeta2) {
        runOnUiThread {
    ...
    override fun setBreak(workoutPos: Int, nextExerciseMeta: ExerciseMeta2, breakLength: Int) {
        runOnUiThread {
            workoutPagerOSVP.setCurrentItem(WORKOUT_INDEX_FIRST + workoutPos, true)
