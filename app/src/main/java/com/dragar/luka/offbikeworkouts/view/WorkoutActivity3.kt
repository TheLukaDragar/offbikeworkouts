...
import com.dragar.luka.offbikeworkouts.presenter.WorkoutService3
import com.dragar.luka.offbikeworkouts.view.WorkoutPagerAdapter3.Companion.SUCCESS_INDEX_FROM_END
import com.dragar.luka.offbikeworkouts.view.WorkoutPagerAdapter3.Companion.WORKOUT_INDEX_FIRST
import kotlinx.android.synthetic.main.activity_workout3.*
...
class WorkoutActivity3 : AppCompatActivity(), ServiceConnection, WorkoutContract3.View {
    companion object {
    ...
    private inner class PausePlayListener : View.OnClickListener {
        override fun onClick(p0: View?) {
            this@WorkoutActivity3.presenterService?.get()?.togglePlayPause()
        }
    }
    ...
    private var presenterService: WeakReference<WorkoutService3>? = null
    private var restoredState: Parcelable? = null
    private var ttsEnabled = false
    private var workout: Workout3? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        workout = intent.getParcelableExtra(WORKOUT_KEY3)
//        workout?.customColor?.toInt()?.let { headerBox.setBackgroundColor(it) } //todo make sure color is applied on restore as well
    ...
    override fun onStart() {
        ...
        bindService(
                Intent(this, WorkoutService3::class.java),
    ...
    override fun onStop() {
        super.onStop()
        startService(Intent(this, WorkoutService3::class.java)) //todo perhaps not at all necessary
    ...
    override fun onServiceConnected(p0: ComponentName?, binder: IBinder) {
        presenterService = (binder as WorkoutService3.WorkoutBinder).service
        presenterService?.let {
            workoutPagerOSVP.adapter = WorkoutPagerAdapter3(supportFragmentManager, workout!!) //todo get workout from binder, not from activity, maybe (though I use the workout here to start the service, so maybe not? but the service may have a different workout)
    ...
    private fun setOnClickListeners(service: WorkoutService3?) {
        runOnUiThread {
    ...
    private val curWorkoutFragment: WorkoutFragment3?
        get() { //todo too inefficient
            val curFragment = (workoutPagerOSVP.adapter as WorkoutPagerAdapter3)
                    .getFragment(workoutPagerOSVP.currentItem) as WorkoutFragment3?
            if (curFragment == null) {
                needsRerender = true
            }
